...
pub fn getBootrom(b: *std.Build, rom: BootROM, storage: Storage) Stage2Bootloader {
    const rom_exe = switch (rom) {
        ...
        .blob => |blob| return Stage2Bootloader{
            ...
            rom_exe.setLinkerScript(.{ .path = b.fmt("{s}/src/bootroms/shared/stage2{s}.ld", .{build_root,suffix}) });
            rom_exe.addAssemblyFile(.{ .path = rom_path });
            rom_exe.addAssemblyFile(.{ .path = b.fmt("{s}/src/bootroms/shared/exit_from_boot2{s}.S", .{build_root,suffix}) });
